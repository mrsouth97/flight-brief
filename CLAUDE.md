...
## DGR (V6.04)
- Tab **DGR** (V6.06) = checklist NOTOC nhiều DG theo **FCD 919 DG Transportation Guide R02**: Step 1 header NOTOC so với OFP (tick); mỗi DG nhập UN, Class(sub), PG, PI, Net qty, số kiện, ERG, ULD, Position. Step 2 so cột C/E/N và net qty/kiện với cột J (PI bắt đầu Y → cột H); Step 3–4 sai khác → tick "SP cho phép"; Step 5 DGM 2.5.8 a); Step 6 không có ULD → Reject; Step 7–8 cùng position → bảng DGR 9.3.A (mọi nhãn, UN3480/3090/3551 nhóm LI, cùng UN không cần cách ly) → tick PIC confirm; Step 9 ACCEPT + ERG drill (Doc 9481). Dữ liệu Blue Pages đọc không được → CHECK (không Reject).
