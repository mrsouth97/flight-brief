...
- Tab WEATHER hiện MINIMA OK / BELOW MINIMA / Not Found cho DEST, DEST ALT, ENR ALT, FUEL ERA, EDTO ALT. **DEP (V6.26):** không nhập minima — mọi sân DEP của VNA đều có landing CAT I → RWY cất cánh trong OFP (`expectedDepRunway`), ILS CAT I, VIS/RVR ≥ 550 m, không ceiling; TEMPO theo 8.1.2-6 như DEST; BELOW → "T/O ALTN required" kèm dòng `TAKEOFF ALTN:` của OFP (`o.takeoffAltn`, "NOT REQD" hoặc ICAO). Form minima: ô DH/MDH không bắt buộc khi approach là ILS/CAT II/III (8.1.2-7 vẫn lấy MDH từ NPA cùng runway). Hiển thị (V6.26): dưới tên sân ghi approach dùng để so (`minimaUsed`, vd "ILS CAT I 27L", hoặc "Minima: Not Found"); minima OK → không có dòng dưới TAF, chỉ tô VIS xanh (BELOW → VIS đỏ + dòng BELOW MINIMA đỏ) — `minimaCheck` trả status, `formatWxVisibility(text,status)`.
...
## DGR (V6.04)
- Tab **DGR** (V6.07): Step 1 header NOTOC so với OFP (tick). Mỗi dòng "Item N" có ô UN/ID + chọn PG (tuỳ chọn, lọc dòng theo PG; V6.08) → hiện tất cả tên của UN đó, nút ✕ ẩn tên không liên quan (↺ để hiện lại), nút **Select** chọn đúng tên theo NOTOC (V6.09; chỉ còn 1 tên thì tự chọn) → hiện thẻ xanh thông tin Blue Pages để phi công tự đối chiếu với NOTOC giấy (Class/sub risk C, PG E, EQ, Ltd Qty G/H, PAX I/J, CAO K/L, ERG N); nút SP bấm để xem nội dung DGR 4.4 (`sp` trong dgr.json: đủ 243 SP từ V6.25 — A1 lấy lại từ PDF, A17/A117/A132/A165/A176 tách khỏi SP đứng trước, A160 = Type B (đoạn cũ gắn nhãn A160 là phần cuối A146); chạm vào ô chữ để đổi EN/VI; `spVi` V6.25 viết lại theo **ý nghĩa với phi công**: 1–2 câu + dòng "→ hành động", SP chỉ liên quan người gửi hàng ghi "Không ảnh hưởng đến phi công"; ô SP giữ mở khi thẻ vẽ lại (`it.spOpen`)); kiểm tra DGM 2.5.8 → "NOT ACCEPTED ON VNA" đỏ (kể cả chỉ CAO). Trạng thái theo PG: chưa chọn PG mà chỉ PG I bị chặn → vàng "PG I NOT ACCEPTED — select PG". Tên có dữ liệu giống hệt được gộp 1 thẻ (V6.10). Khung **Same position check · Table 9.3.A** (V6.11, một khung, không có "add position"): V6.19: không cần tích, tự kiểm tra 9.3.A giữa tất cả item đã nhập → kết quả ngay trong khung (Can be loaded in the same position / SEGREGATE + bảng từng cặp). Summary còn: item bị chặn, ERG drill.
- Khung **Loadsheet** (dưới Position Check từ V6.17): đọc loadsheet mới nhất (R cao nhất) của chuyến → NOTOC YES/NO + bảng Position → mã IMP (bảng `IMP_CODES` theo DGM 3.2.2.1 b / DGR App. B 2.2.4); cảnh báo đỏ nếu 2 mã cùng vị trí phải cách ly (9.3.A), ICE ngoài BULK (5), MAG ngoài compartment 3/4. V6.23: bỏ tên file loadsheet; không có limit → "Nil"; ICE chỉ "BULK only"; đạn: chỉ hành lý gửi, báo PIC số lượng + vị trí (DGM Table 4.2 no. 2). V6.22: mô tả mã IMP ghi kèm số UN, mã miễn NOTOC ghi "— no NOTOC" (ELI/ELM/RDS/RRE/REQ); có AVI trên loadsheet → giới hạn ICE hiện "max 100 kg with live animals … ≥ 3 m from AVI" (DGM 2.5.10.2 c · 3.5.5). V6.21: gộp mã IMP của load message + mã DG trong SI thành **một bảng** Position · Code · DG · Limit (trùng vị trí+mã chỉ hiện 1 lần; mã SI có vị trí cũng được xét cách ly/ICE/MAG). V6.20: bảng **SI** — đọc các dòng free text SI (layoutText), tìm mã IMP (`RLM/0.1`, vị trí lấy từ dòng `42P/PMC…` ngay trước hoặc `POS 43R`), mã hành khách WCLB/WCBW/WCBD/OXYG, đạn (BULLET/CARTRIDGE/AMMUNITION; súng không phải DG) → giải nghĩa + cột Limit nếu có (ICE, MAG theo DGM 2.5.10.2; xe lăn DGR 2.3.2.2–2.3.2.4; oxy 2.3.4.1; đạn Table 2.3.A).
...
- Nguồn: `~/Downloads/DANGEROUS GOODS REGULATIONS.pdf` (67th ed. 2026, OCR scan). Script tách: `~/Downloads/FlightBriefNativeRemote/tools/dgr/`. Parser V6.18 sửa lỗi dòng scan nghiêng (ghép nhầm số liệu sang mục kế bên); 229 mục cờ ⚠ đã được đối chiếu từng ô với ảnh trang scan (26/09/2026); sau đó đối chiếu **toàn bộ** Blue Pages (mọi ô số liệu + tên, 25/09/2026): sửa 246 mục số liệu, 210 tên, thêm 13 mục bị sót → 2667 mục, 0 cờ ⚠. Cờ ⚠ vẫn hiện nếu sau này dữ liệu đọc chưa chắc. Mỗi năm có DGR mới: chạy lại script, build & cài app.
